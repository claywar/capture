...
pv.commands = {
    ['track'] = function(args)
        if args[1] == 'stop' or args[1] == 's' then
            pv.tracking = T { incoming = S {}, outgoing = S {} }
            pv.tracker:hide()
            return
        ...
        for i = 1, args:length() / 2 do
            ...
            value = value:number() or value == 'true' and true or value ~= 'false' and value or false
    ...
    ['scan'] = function(args)
        ...
        if not args[2] and pv.direction_strings[args[1]] then
            error(
                'Specify an argument to scan for: //pv scan incoming|outgoing|both [known|unknown|hybrid] <val>')
            return
        end
...
do
    ...
    top_row:enclose(pv.colors['hexborder'], '\\cr')
    ...
    local line_replace = {}
    for i = 0x01, 0x10 do
        line_replace[i] = pv.colors.hexborder .. '%%%%3X |' .. string.rep(' %%s%.2X', i) .. pv.colors.gray ..
                              string.rep(' --', 0x10 - i) .. pv.colors.hexborder .. '  %%%%3X | ' ..
                              '%%%%%%%%s\n'
    end
    local short_replace = {}
    for i = 0x01, 0x10 do
        short_replace[i] = string.rep('%%s%s', i) ..
                               (i < 0x10 and pv.colors.gray .. string.rep('-', 0x10 - i) or '')
    end
    ...
    string.pv_hexformat = function(str)
        ...
        for i = 0, ((length - 1) / 0x10):floor() do
            ...
            str_table[i + 1] = line_replace[bytes]:format(unpack(partial_str)):format(partial_col:unpack())
                :format(i, i):format(short_replace[bytes]:format(unpack(char_table)):format(
                    partial_col:unpack()))
            from = to + 1
            to = to + 0x10
        end
        return string.format('%s%s\\cr', top_row, table.concat(str_table))
    end
end
...
do
    ...
    local line_replace = {}
    for i = 0x01, 0x10 do
        line_replace[i] = '    %%%%3X |' .. string.rep(' %.2X', i) .. string.rep(' --', 0x10 - i) ..
                              '  %%%%3X | ' .. '%%s\n'
    end
    local short_replace = {}
    for i = 0x01, 0x10 do
    ...
    string.pv_hexformat_file = function(str, byte_colors)
        ...
        for i = 0, ((length - 1) / 0x10):floor() do
            ...
            local bytes = (length - from + 1):min(16)
            str_table[i + 1] = line_replace[bytes]:format(unpack(partial_str)):format(
                short_replace[bytes]:format(unpack(char_table))):format(i, i)
            from = to + 1
            to = to + 0x10
        end
...
pv.filter_settings = function(field)
    return field.label:startswith('_junk') and pv.settings.Show.Junk or field.label:startswith('_unknown') and
               pv.settings.Show.Unknown or pv.settings.Show.Known
end
...
pv.track_packet = function(dir, id, data, modified, injected, blocked)
    ...
    if fields then
        ...
        for field, index in fields:it() do
            ...
            if filter_pass then
                packet._lines:append(string.format('%s: %s${%s|-}%s\\cr', field.label, color, field.label,
                    (field.fn and string.format('${_f_%s}', field.label) or '')))
            end
        end
        ...
        if pv.settings.CheckConst then
            for field in fields:it() do
                if field.const and field.const ~= packet[field.label] then
                    print(string.format('Const violation found in %s packet 0x%.3X, field %s: %s ≠ %s',
                        packet._dir, packet._id, field.label, tostring(packet[field.label]),
                        tostring(field.const)))
                end
            end
        end
        ...
        for field in fields:filter(pv.filter_settings):filter(
            string.startswith - { 'data' } .. table.get - { 'ctype' }):it() do
            packet[field.label] = '…'
        end
    end
    ...
    packet._hexid = string.format('0x%.3X', id)
    packet._hextable = packet._raw:pv_hexformat(pv.tracking.byte_colors) -- The fully colored hex table

    packet._display_padding = string.rep(' ', packet._id:log(10):floor() - packet._size:log(10):floor() + 2)
...
do
    local mods = {
        [true] = { [true] = ' (Injected, Blocked)', [false] = ' (Injected)' },
    ...
    pv.log_packet = function(dir, id, data, modified, injected, blocked)
        ...
        if not force and (pv.logging.mode ~= 'hybrid' and
            (pv.logging.mode == 'known' and name == 'Unknown' or pv.logging.mode == 'unknown' and name ~=
                'Unknown')) then
            return
        end
                ...
                if fields then
                    ...
                    for field in fields:filter(pv.filter_settings):it() do
                        field_data = field_data ..
                                         string.format('%s: %s%s\n', field.label,
                                tostring(packet[field.label]), field.fn and
                                    string.format(' (%s)', field.fn(packet[field.label], data)) or '')
                    end
                end
            end

            local timestamp = pv.settings.LogTimestamp and string.format('[%s] ', os.date('%Y-%m-%d %X')) or
                                  ''

            if pv.settings.mode == lib.mode.CAPTURE then
                pv.write_packet_to_file(pv.file.capture.full,
                    string.format('%s%s\n%s%s\n', timestamp, header, hex_data, field_data), 'full')
                pv.write_packet_to_file(pv.file.capture[dir], string.format('%s%s\n%s%s\n', timestamp,
                    string.format('Packet 0x%.3X%s', id, mod_str), hex_data, field_data), dir)
                pv.write_packet_to_file(files.new(pv.vars.capture_root ..
                                                      string.format('%s/0x%.3X.log', dir, id), true),
                    string.format('%s%s\n%s%s\n', timestamp, mod_str, hex_data, field_data), dir .. '_' .. id)
            elseif pv.settings.mode >= lib.mode.INFO then
                pv.write_packet_to_file(pv.file.full, string.format('%s%s\n%s%s\n', timestamp, header,
                    hex_data, field_data), 'full')
                pv.write_packet_to_file(pv.file[dir], string.format('%s%s\n%s%s\n', timestamp, string.format(
                    'Packet 0x%.3X%s', id, mod_str), hex_data, field_data), dir)
                pv.write_packet_to_file(files.new(pv.settings.file_path ..
                                                      string.format('%s/%s/0x%.3X.log', pv.vars.my_name, dir,
                        id), true), string.format('%s%s\n%s%s\n', timestamp, mod_str, hex_data, field_data),
                    dir .. '_' .. id)
            end
        end
    end
    ...
    if not pv.write_lock[stream_id] then -- Make sure we don't try to push into a string that might about to be emptied
        ...
        if not pv.write_scheduled[stream_id] then
            ...
            coroutine.schedule(function()
                pv.write_to_file(file_stream, stream_id)
            end, 0.5)
        end
    else
        -- Try again in a little bit
        coroutine.schedule(function()
            pv.write_packet_to_file(file_stream, string, stream_id)
        end, 0.1)
    end
end
...
pv.scan_packet = function(dir, id, data, modified, injected, blocked)
    ...
    print(string.format('Match found for %s in %s packet %u (0x%.3X) at byte %u (0x%.2X).',
        tostring(pv.scan.value:unpack(pv.scan.pack)), dir, id, id, from, from))
end
...
pv.register_packet = function(dir, id, data, modified, injected, blocked)
    ...
    if pv.tracking.active and pv.tracking[dir]:contains(id) and
        (pv.tracking.once == nil or pv.tracking.once == true) then
    ...
    if pv.logging.output and (pv.logging.direction == 'both' or dir == pv.logging.direction) and
        pv.logging.filter:contains(id) then
...
pv.display_packet = function(index)
    if index > pv.saved_packets.n then
        if pv.saved_packets.n == 0 then
            ...
            error(string.format('Only %u packet%s recorded.', pv.saved_packets.n,
                pv.saved_packets.n > 1 and 's' or ''))
        end
...
pv.initialize = function()
    ...
    pv.tracking = T { incoming = S {}, outgoing = S {} }
    pv.logging = T {}
    pv.scan = T { active = true, mode = 'unknown', value = nil }
