...
hptrack.processDamage = function(mob)
    ...
    if mob.hpp > 0 then
        if percent_gap > 1 then
            local new_interval_slice = { ['percent_gap'] = percent_gap, ['damage'] = mob.buffered_damage }
...
hptrack.addHPToMobList = function(db, mob)
    ...
    if not zone_db then
        db.zone[hptrack.vars.current_zone] = {
            info = {},
            _meta = { key = hptrack.vars.current_zone, num = 0, loaded_num = 0, parent = true },
        }
    ...
    if not group_db then
        zone_db.info[mob_name] = { info = {}, _meta = { key = mob_name, parent = true, num = 0 } }
        group_db = zone_db.info[mob_name]
    end

    local mob_db = group_db.info[mob_id]
    if not mob_db then
        group_db.info[mob_id] = { info = {}, _meta = { key = mob_id, num = 0, parent = true } }
...
function hptrack.setupZone(zone, zone_left)
    ...
    if hptrack.settings.mode == lib.mode.CAPTURE then
        hptrack.file.capture.simple = lib.fileOpen(hptrack.vars.capture_root .. 'simple/' .. zone_name ..
                                                       '.log')
    end
end
...
hptrack.initialize = function()
    ...
    hptrack.msg_types = {
        ...
        [6] = { [110] = { 'Ability Dmg', 1 }, [263] = { 'AOE (Recovery)', -1 }, [264] = { 'AOE Damage', 1 } },
    }
    ...
    hptrack.file.simple = lib.fileOpen(hptrack.settings.file_path .. hptrack.vars.my_name ..
                                           '/logs/simple.log')
