...
caplog.logMessage = function(message)
    if caplog.settings.mode >= lib.mode.PASSIVE then
        ...
        if caplog.settings.timestamps then
            timestamp = string.gsub(os.date('[%H:%M:%S] '), '%${([%l%d_]+)}', {})
        end
...
caplog.startCapture = function()
    ...
    local filename = string.format('%s_%.4u.%.2u.%.2u.txt', caplog.vars.my_name, date.year, date.month,
        date.day)
...
caplog.initialize = function()
    ...
    caplog.template = {
        blocked = caplog.color.box.SYSTEM .. '[CL] ' .. caplog.color.box.SYSTEM .. 'T: ' ..
            caplog.color.box.LINKSHELL .. '%s' .. caplog.color.box.SYSTEM .. 'P: ' ..
            caplog.color.box.LINKSHELL .. '%s' .. caplog.color.box.SYSTEM .. 'L: ' ..
            caplog.color.box.LINKSHELL .. '%s',
    }
