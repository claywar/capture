...
lib.mode = { OFF = 0, INFO = 1, PASSIVE = 2, ACTIVE = 3, CAPTURE = 4 }
...
lib.formatDatabaseEntry = function(info, format, indent)
    ...
    local format_args = { [1] = tostring(info[format._key.field]):gsub('([\'"\\])', '\\%1') }
...
lib.fileOpen = function(path)
    local file = { stream = files.new(path, true), locked = false, scheduled = false, buffer = '' }
    return file
end
...
lib.loadTable = function(db_table)
    ...
    if type(db_table) == 'table' then
        ...
        return { ['info'] = info, ['_meta'] = { num = total_num, parent = nested } }, total_num
    else
        return db_table, 1
    end
end
...
lib.performColorTest = function(box_settings)
    ...
    if box_settings then
        ...
        texts.pos(color_box, windower.get_windower_settings().x_res * 1 / 3,
            windower.get_windower_settings().y_res * 2 / 3)
        return color_box
    end
    return false
end
...
lib.capture = function(addon, args)
    if args[1] then
        ...
        elseif toggle == 'stop' then
            ...
            lib.msg(addon,
                'Usage: //' .. addon.info.commands[#addon.info.commands] .. ' capture [start | stop]')
        end
    else
...
lib.startCapture = function(addon, file_root)
    if not (addon.settings.mode == lib.mode.CAPTURE) then
        ...
        if not file_root then
            ...
            file_root = file_root:format(date.year, date.month, date.day, date.hour, date.min,
                addon.vars.my_name)
        end
...
lib.checkLibVer = function(addon)
    if lib.version ~= addon.info.lib_version then
        lib.msg(addon,
            'LIBRARY WARNING - Expected: v' .. addon.info.lib_version .. '. Current: v' .. lib.version)
    end
end
...
lib.displayVer = function(addon)
    ...
    lib.msg(addon, 'Library: v' .. lib.version .. ' [' .. lib.date .. '] (Expected: v' ..
        addon.info.lib_version .. ')')
end
    ...
    if addon.vars.show_box then
        if info then
            ...
            for i = 1, #addon.vars.boxes do
                texts.pos(addon.vars.boxes[i], addon.vars.box_positions[i + 1].x,
                    addon.vars.box_positions[i + 1].y)
            end
            ...
            if addon.settings.auto_hide and not addon.vars.hide_ticking then
                coroutine.schedule(function()
                    lib.updateBox(addon)
                end, addon.settings.auto_hide_time + 0.2)
...
lib.testBox = function(addon, info)
    if not addon.vars.testing_box then
        ...
        addon.vars.box_positions = { [1] = { x = addon.vars.initial_x, y = addon.vars.initial_y } }
        for i = 2, 10 do
            addon.vars.box_positions[i] = {
                x = addon.vars.initial_x,
                y = addon.vars.initial_y -
                    (((addon.settings.box.text.size + addon.settings.box.spacing) * 4) * (i - 1)),
            }
        end
    end
end
