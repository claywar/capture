...
eventview.setupZone = function(zone)
    ...
    eventview.file.simple = lib.fileOpen(
        eventview.settings.file_path .. eventview.vars.my_name .. '/simple/' .. current_zone .. '.log')
    eventview.file.raw = lib.fileOpen(eventview.settings.file_path .. eventview.vars.my_name .. '/raw/' ..
                                          current_zone .. '.log')

    if eventview.settings.mode == lib.mode.CAPTURE then
        eventview.file.capture.simple = lib.fileOpen(
            eventview.vars.capture_root .. 'simple/' .. current_zone .. '.log')
        eventview.file.capture.raw = lib.fileOpen(eventview.vars.capture_root .. 'raw/' .. current_zone ..
                                                      '.log')
    end
end
...
eventview.buildSimpleString = function(info)
    ...
    return
        string.format(info.dir .. ' ' .. packet_type.text .. packet_type.simple_string, unpack(simple_info))
end
    ...
    if eventview.packets[id] and (eventview.packets[id].dir == dir) then
        ...
        elseif update_packet['Option Index'] then
            ...
            if update_packet['Menu ID'] then
                ...
                if (not update_packet['Menu Zone']) or (update_packet['Menu Zone'] <= 0) then
                    info.params, info.param_string = eventview.getParams(string.sub(data:hex(),
                        (0x08 * 2) + 1, (0x28 * 2)))
                end
            elseif id ~= 0x032 then
                info.params, info.param_string = eventview.getParams(
                    string.sub(data:hex(), (0x04 * 2) + 1, (0x24 * 2)))
            end
        end
        ...
        if eventview.settings.mode >= lib.mode.PASSIVE then
            ...
            if eventview.settings.mode == lib.mode.CAPTURE then
                ...
                lib.fileAppend(eventview.file.capture.raw,
                    simple_string .. '\n' .. data:hexformat_file() .. '\n')
            end
        end
...
eventview.initialize = function()
    ...
    eventview.template = {}
    eventview.template.EVENT = ' ${dir|%s} ${type|%s}' .. eventview.color.box.SYSTEM ..
                                   '[ID View]\n Event: ${event|%s}' .. eventview.color.box.SYSTEM ..
                                   ' Actor: ${actor|%s}\n \n '
    eventview.template.ZONE = ' ${dir|%s} ${type|%s}' .. eventview.color.box.SYSTEM ..
                                  '[ID View]\n Event: ${event|%s}' .. eventview.color.box.SYSTEM ..
                                  ' Zone: ${actor|%s}\n \n '
    eventview.template.EVENT_PARAMS = ' ${dir|%s} ${type|%s}' .. eventview.color.box.SYSTEM ..
                                          '[ID View]\n Event: ${event|%s}' .. eventview.color.box.SYSTEM ..
                                          ' Actor: ${actor|%s}\n ' .. eventview.color.box.SYSTEM .. 'P: ' ..
                                          eventview.color.box.PARAMS ..
                                          '${p1|%s} ${p2|%s} ${p3|%s} ${p4|%s}\n    ${p5|%s} ${p6|%s} ${p7|%s} ${p8|%s}'
    eventview.template.OPTION = ' ${dir|%s} ${type|%s}' .. eventview.color.box.SYSTEM ..
                                    '[ID View]\n Event: ${event|%s} ' .. eventview.color.box.SYSTEM ..
                                    'Actor: ${actor|%s}\n' .. eventview.color.box.SYSTEM ..
                                    ' Option: ${option|%s}\n '
    eventview.template.UPDATE = ' ${dir|%s} ${type|%s}' .. eventview.color.box.SYSTEM .. '[ID View]\n' ..
                                    ' P: ' .. eventview.color.box.PARAMS ..
                                    '${p1|%s} ${p2|%s} ${p3|%s} ${p4|%s}\n    ${p5|%s} ${p6|%s} ${p7|%s} ${p8|%s}\n '
    ...
    eventview.vars.box_positions = { [1] = { x = eventview.vars.initial_x, y = eventview.vars.initial_y } }
    for i = 2, 10 do
        eventview.vars.box_positions[i] = {
            x = eventview.vars.initial_x,
            y = eventview.vars.initial_y -
                (((eventview.settings.box.text.size + eventview.settings.box.spacing) * 4) * (i - 1)),
        }
    end
    ...
    eventview.file.simple = lib.fileOpen(eventview.settings.file_path .. eventview.vars.my_name ..
                                             '/logs/simple.log')
    eventview.file.raw = lib.fileOpen(eventview.settings.file_path .. eventview.vars.my_name ..
                                          '/logs/raw.log')
    ...
    eventview.packets = {
        [0x00A] = {
            ...
            string_params = function(info)
                return { [1] = info.actor, [2] = info.event }
            end,
        ...
        [0x034] = {
            ...
            box_color = eventview.color.box.EVENT_HEADER,
            log_color = eventview.color.log.EVENT_HEADER,
            log_string = eventview.h.actor .. '%s, ' .. eventview.h.event .. '%s, ' .. eventview.h.params ..
                '%s',
            simple_string = ' NPC: %s\nEvent: %s\nParams: %s',
            string_params = function(info)
                return { [1] = info.actor, [2] = info.event, [3] = info.param_string }
            end,
        ...
        [0x036] = {
            ...
            simple_string = ' NPC: %s\nMessage: %s',
            string_params = function(info)
                return { [1] = info.actor, [2] = info.message }
            end,
        ...
        [0x05B] = {
            ...
            log_string = eventview.h.actor .. '%s, ' .. eventview.h.event .. '%s, ' .. eventview.h.option ..
                '%s',
            simple_string = ' NPC: %s\nEvent: %s\nOption: %s',
            string_params = function(info)
                return { [1] = info.actor, [2] = info.event, [3] = info.option }
            end,
    ...
    windower.register_event('zone change', function(new, old)
        eventview.setupZone(new)
    end)
    windower.register_event('outgoing chunk', function(id, data, modified, injected, blocked)
        eventview.checkChunk('OUTGOING >', id, data)
    end)
    windower.register_event('incoming chunk', function(id, data, modified, injected, blocked)
        eventview.checkChunk('INCOMING <', id, data)
    end)
