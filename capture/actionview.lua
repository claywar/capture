...
actionview.buildChatlogString = function(info)
    local chatlog_string = actionview.h.name .. actionview.h.category .. '%s ' .. actionview.h.id .. '%s ' ..
                               actionview.h.animation .. '%s ' .. actionview.h.message .. '%s'
...
actionview.buildSimpleString = function(info)
    ...
    return string.format(simple_info, info.actor, info.actor_name, info.name, info.category, info.id,
        info.animation, info.message)
end
...
actionview.addActionToMobList = function(db, result)
    ...
    if not cat_db then
        mob_db.info[result.category] =
            { info = {}, _meta = { key = result.category, num = 0, parent = true } }
...
actionview.writeDatabases = function(zone_left)
    ...
    if actionview.vars.new_db_info then
        ...
        if actionview.settings.mode == lib.mode.CAPTURE then
            ...
            if actionview.db.data.capture.zone[zone_left]._meta.num > actionview.db.data.capture.zone[zone_left]._meta.loaded_num then
                ...
                lib.writeDatabase(actionview, actionview.db.data.capture.zone, zone_left,
                    actionview.file.capture.old_zone)
                actionview.db.data.capture.zone[zone_left]._meta.loaded_num =
                    actionview.db.data.capture.zone[zone_left]._meta.num
            end
        end
        ...
        for i = 1, 15 do
            ...
            if actionview.settings.mode == lib.mode.CAPTURE then
                if actionview.db.data.capture.category[i]._meta.num >
                    actionview.db.data.capture.category[i]._meta.loaded_num then
                    had_new = true
                    lib.writeDatabase(actionview, actionview.db.data.capture.category, i,
                        actionview.file.capture.category[i])
                    actionview.db.data.capture.category[i]._meta.loaded_num =
                        actionview.db.data.category[i]._meta.num
                end
            end
        end
    end
...
actionview.prepareZoneDatabase = function(zone)
    ...
    if actionview.settings.mode == lib.mode.CAPTURE then
        ...
        if actionview.file.capture.zone:exists(path) then
            ...
            actionview.db.data.capture.zone[zone]._meta.loaded_num =
                actionview.db.data.capture.zone[zone]._meta.num
        end
    end
end
...
actionview.setupZone = function(zone)
    ...
    actionview.file.simple =
        lib.fileOpen(actionview.settings.file_path .. 'simple/' .. current_zone .. '.log')

    if actionview.settings.mode == lib.mode.CAPTURE then
        actionview.file.capture.simple = lib.fileOpen(actionview.vars.capture_root .. 'simple/' ..
                                                          current_zone .. '.log')
    end
end
