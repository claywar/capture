    ...
    ['always_widescan'] = function(args)
        if args[1] then
            if string.lower(args[1]) == 'on' then
                ...
                npcl.doWidescan()
                npcl.display(
                    'Always auto-widescanning! (Persists across zones; packets sent when normally impossible!)')
...
npcl.getBasicNpcInfo = function(npc_id, raw_name, look, packet)
    ...
    if not npc.scannable then
        npc.level = 'X'
        npc.widescan = { index = npc.index, seen = 'X', level = 'X' }
    end
...
npcl.basicNpcInfoString = function(npc)
    return string.format(
        'NPC ID: %d\n  Name: %s\n  POLUtils_Name: %s\n  Type: %s\n  XYZR: %.3f, %.3f, %.3f, %d\n', npc.id,
        npc.name, npc.polutils_name, npc.type, npc.x, npc.y, npc.z, npc.r)
end
...
npcl.formatLogRaw = function(npc, mask, data)
    ...
    local log_string = string.format('%s  Mask: %s\n%s\n', info_string, mask, hex_data)
...
npcl.formatLogPacketToTable = function(npc, data)
    ...
    log_string = log_string .. string.format(
        ...
        npc.id, npc.name:gsub('([\'"\\])', '\\%1'):gsub('%s', '_'),
        npc.polutils_name:gsub('([\'"\\])', '\\%1'), npc.type, npc.index, npc.x, npc.y, npc.z, npc.r,
        npc.flag, npc.speed, npc.speedsub, npc.animation, npc.animationsub, npc.namevis, npc.status,
        npc.flags, npc.name_prefix, npc.look)
...
npcl.formatDatabaseEntry = function(npc)
    ...
    database_entry = database_entry .. string.format('[\'id\']=%d, %s %s %s %s ', npc['id'], npcl.padField(
        'name', npc['name']:gsub('([\'"\\])', '\\%1'):gsub('%s', '_')), npcl.padField('polutils_name',
        npc['polutils_name']:gsub('([\'"\\])', '\\%1')), npcl.padField('type', npc['type']),
    ...
    database_entry = database_entry ..
                         string.format(' %s %s %s %s %s %s %s %s %s %s %s %s %s %s [\'raw_packet\']="%s",',
            npcl.padField('x', string.format('%.3f', npc['x'])),
            npcl.padField('y', string.format('%.3f', npc['y'])),
            npcl.padField('z', string.format('%.3f', npc['z'])), npcl.padField('r', npc['r']),
            npcl.padField('flag', npc['flag']), npcl.padField('speed', npc['speed']),
            npcl.padField('speedsub', npc['speedsub']), npcl.padField('animation', npc['animation']),
            npcl.padField('animationsub', npc['animationsub']), npcl.padField('namevis', npc['namevis']),
            npcl.padField('status', npc['status']), npcl.padField('flags', npc['flags']),
            npcl.padField('name_prefix', npc['name_prefix']), npcl.padField('look', npc['look']),
            npc['raw_packet'])
    database_entry = database_entry .. '},\n'
...
npcl.scheduleDatabaseWrite = function()
    ...
    if not npcl.vars.write_scheduled then
        ...
        npcl.vars.scheduled_write_coroutine = coroutine.schedule(function()
            npcl.writeZoneDatabase(npcl.vars.current_zone)
        end, 60)
    end
end
...
npcl.writeZoneDatabase = function(zone_left)
    ...
    if npcl.vars.new_npcs_seen then
        ...
        local melded, updated_npcs = lib.meldDatabases(npcl.db.main.npc_info, file_zone_database,
            npcl.shouldUpdate)
        ...
        if npcl.settings.mode == lib.mode.CAPTURE then
            ...
            local capture_melded, capture_updated_npcs =
                lib.meldDatabases(npcl.db.capture.npc_info, capture_zone_database, npcl.shouldUpdate)
    ...
    if npcl.file.database:exists(path .. '.lua') then
        ...
        for id, npc in pairs(database) do
            ...
            if npc.level and (npc.level ~= '?') then
                memory[id].widescan = { checked = true, seen = npc.level, level = npc.level }
            end
            if npc.index then
...
npcl.setupZone = function(zone, zone_left)
    ...
    npcl.file.packet_table = lib.fileOpen(npcl.settings.file_path .. npcl.vars.my_name .. '/tables/' ..
                                              zone_name .. '.lua')
    npcl.file.full = lib.fileOpen(npcl.settings.file_path .. npcl.vars.my_name .. '/logs/' .. zone_name ..
                                      '.log')
    npcl.file.widescan = lib.fileOpen(npcl.settings.file_path .. npcl.vars.my_name .. '/widescan/' ..
                                          zone_name .. '.log')
    ...
    npcl.db.main.npc_info, npcl.db.main.index_info = npcl.loadDatabase(
        npcl.settings.file_path .. 'database/' .. zone_name, zone)
    npcl.db.main.widescan.num_new = 0
    npcl.db.main.widescan.new = {}

    if npcl.settings.mode == lib.mode.CAPTURE then
        npcl.db.capture.npc_info, npcl.db.capture.index_info =
            npcl.loadDatabase(npcl.vars.capture_root .. 'database/' .. zone_name, zone)
        npcl.db.capture.widescan.num_new = 0
        npcl.db.capture.widescan.new = {}
        npcl.file.capture.packet_table = lib.fileOpen(npcl.vars.capture_root .. 'tables/' .. zone_name ..
                                                          '.lua')
        npcl.file.capture.full = lib.fileOpen(npcl.vars.capture_root .. 'logs/' .. zone_name .. '.log')
        npcl.file.capture.widescan =
            lib.fileOpen(npcl.vars.capture_root .. 'widescan/' .. zone_name .. '.log')
    end
        ...
        if not saw_mask then
            ...
            if (mask == 0x57) or (mask == 0x0F) or (mask == 0x07) then
                ...
                if index_info.level then
                    npc.widescan = { id = npc.id, index = npc.index, level = index_info.level }
                ...
                if (not new_for.npc_info[npc_id]) or
                    (new_for.npc_info[npc_id] and (not new_for.npc_info[npc_id].polutils_name)) then
                    coroutine.schedule(function()
                ...
                coroutine.schedule(function()
                    npcl.logNpc(npc, packet['Mask'], data)
                end, 3.3)
            end
        end
    end
...
npcl.formatWidescanInfo = function(npc)
    ...
    if level and npc.name then
        log_string = log_string ..
                         string.format('[\'id\']=%d, [\'name\']="%s", [\'index\']=%d, [\'level\']=%d', npc.id,
                npc.name:gsub('([\'"\\])', '\\%1'):gsub('%s', '_'), npc.index, level)
        log_string = log_string .. '},\n'
    end
    return log_string
end
...
npcl.preRender = function()
    if npcl.settings.show_bar then
        if npcl.vars.debug.save_time > 0 then
            npcl.vars.debug.box.save_time = lib.padRight(tostring(
                ((npcl.vars.debug.save_time - os.clock()) % 60):floor()), 2)
        end
        if npcl.vars.debug.ws_time > 0 then
            npcl.vars.debug.box.ws_time = lib.padRight(tostring(
                ((npcl.vars.debug.ws_time - os.clock()) % 60):floor()), 2)
        end
        if npcl.vars.debug.ws_auto > 0 then
            npcl.vars.debug.box.ws_auto = lib.padRight(tostring(
                ((npcl.vars.debug.ws_auto - os.clock()) % 60):floor()), 2)
        end
        ...
        if not info then
            ...
            if mob and mob.id > 0 then
                ...
                if npc and npc.widescan and npc.widescan.seen then
                    npcl.vars.debug.box.npc_level = '\\cs(100,255,100)' ..
                                                        lib.padRight(tostring(npc.widescan.seen), 3) .. '\\cr'
                else
...
npcl.startCapture = function()
    npcl.db.capture = {
        index_info = {},
        npc_info = {},
        seen_masks = { [0x57] = {}, [0x07] = {}, [0x0F] = {} },
        widescan = { num_new = 0, new = {}, seen = {}, checked = {} },
    }
...
npcl.initialize = function()
    ...
    npcl.file.capture = T {}
    npcl.file.compare =
        files.new(npcl.settings.file_path .. npcl.vars.my_name .. '/logs/comparison.log', true)
    ...
    if info then
        ...
        if (npcl.settings.auto_ws_mode == 2) then
            box_text =
                '[NPCL] Mob: ${npc_id|%s} Level: ${npc_level|%s} Save: ${save_time|%2d}s AutoWS: ${ws_auto|%2d} WSCool: ${ws_time|%2d}'
        else
            box_text =
                '[NPCL] Mob: ${npc_id|%s} Level: ${npc_level|%s} Save: ${save_time|%2d} WS: ${ws_time|%2d}'
        end
    end
    ...
    fields = {
        ['always_string'] = { ['name'] = true, ['polutils_name'] = true, ['type'] = true, ['look'] = true },
        ['padding'] = {
            ['name'] = 35,
            ['polutils_name'] = 55,
            ['type'] = 24,
